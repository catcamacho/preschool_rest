...
group_mat = studyhome + '/misc/design_withNC.mat'
proc_cores = 2
...
sbc2_workflow2.connect([(infosource2,datagrabber,[('roi','roi')]),
                        ...
                        (highermodel,datasink,[('t_corrected_p_files','rand_corrp_files_withNC')]),
                        (highermodel,datasink,[('tstat_files','rand_tstat_files_withNC')])
                       ])
sbc2_workflow2.base_dir = workflow_dir
sbc2_workflow2.write_graph(graph2use='flat')
sbc2_workflow2.run('MultiProc', plugin_args={'n_procs': proc_cores})
...
sbc3_workflow2 = Workflow(name='sbc3_workflow')
sbc3_workflow2.connect([(infosource3,selectfiles2, [('roi','roi')]),
                        ...
                        (binarize,datasink,[('binary_file','binary_pval_withNC')]),
                        (mask_tmaps,datasink,[('out_file','masked_tmaps_withNC')]),
                        (clusterize,datasink,[('index_file','cluster_index_file_withNC')]),
                        (clusterize,datasink,[('localmax_txt_file','localmax_txt_file_withNC')])
                       ])
sbc3_workflow2.base_dir = workflow_dir
sbc3_workflow2.write_graph(graph2use='flat')
sbc3_workflow2.run('MultiProc', plugin_args={'n_procs': proc_cores})
